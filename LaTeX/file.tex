...
\\
The heatmap is a great visualization of the developed optimization algorithm in practice, and it also showcases the options for precompensation. These options are further illustrated and explored in the other two Optimize methods: error plotter and tradeoff plot.
...
\subsubsection{Transport over Neighboring Segments}
...
\begin{tabular}{ | c | c | } 
  ...
  Regularization Exponent & -6 $\rightarrow$ 0 \\ 
  \hline
  Compression Range & 1 $\rightarrow$ 10 \\ 
  \hline
\end{tabular}
\end{center}
...
\begin{minted}{python}
#The best padding is: 8
#The best regularization is: 5e-05
#The best compression is: 5
#The best penalty is: 0.7289897979212718
#Root Mean Squared Error (RMSE): 0.075 V
#Max Absolute Error: 0.179 V
\end{minted}

This neighboring transport ramp has a compression factor of 5.
\\
...
\subsubsection{Transport over Non-Neighboring Segments}

In this second example, the optimal non-neighboring segment transport waveform is shown.
...
In all cases of transport waveform ramps, a maximum absolute error of well below 1 volt is best achieved where compressions are under a factor of 10, and with paddings of around 5 to 10. The regularization strength for these values is usually also in the 5e-4 to 2e-2 range, which lies in the middle of the heatmap. 

For much larger compressions, a larger error must be accounted for, as seen in the trade off graphs.
\\
...
Because of this limit of 380 ns or higher resolution (more samples), the waveform would need to be longer in order to have a higher accuracy, which is the opposite of what compression does, and what is desired.
\\
...
\section{Ramp Excitation Simulation}
Next, the ion transport is simulated for excitation expectation. The goal is to see no excitation (phonons) induced during the transport.
This is first tested on the unprecompensated ramp, and later on the precompensated one.

\subsection{Unprecompensated Ramp Simulation}

The goal here is to transport the ion from segment (electrode) 14 to electrode 15. First, the initial location of the ion is found, and it is at the bottom of the potential.

\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{Figures/ionlocation.png}
\caption{Ion initial location at bottom of potential well at segment 14.}
\end{figure}

Then, the aformentioned classes Optimizer and FilterSimulation are imported. There is an additional method inserted into Optimizer that optimizes (min) excitation and (max) compression.
        ...
        for i, pad in enumerate(self.padding_range):
            for j, reg in enumerate(self.reg_exponents):
                for l, coeff in enumerate(self.reg_coeff):
                    ...
                    time, original = FilterSimulation().setup(time_array, waveform,
                        compress, pad)

                    precompensated = FilterSimulation().precompensation(original, 
                        reg_strength, pad)
                    precompensated = precompensated[:len(original)]
                    precompensated = FilterSimulation().trimming(original, 
                        precompensated, pad)
                    precompensated_filtered = FilterSimulation().convolution
                        (precompensated)
                    shift_p = np.arange(len(precompensated_filtered)) * self.time_step
                    precompensated_filtered = FilterSimulation().interpolation(time, 
                        shift_p, precompensated_filtered)
...
This method returns the best regularization strength and padding value for the highest compression and lowest excitation. The code is very similar to the previously detailed optimizer methods, and will not be explained in depth here.
\\
Additionally, another class is built for calculating the excitation of the ion as it is shuttled accross segments.
To get the excitation of the ion, first its velocity and acceleration must be calculated. For this, a verlet integrator is used. The basic  Størmer–Verlet is a second-order differential equation that depends on ion position.
\begin{equation}
   M \ddot{x} (t) = f(x(t)) = - \nabla V(x(t))
\end{equation}
Here, $t$ is time, $x(t)$ is the ensemble of position vectors, $V$ is the scalar potential function, $F$ is the negative gradient of potential, and $M$ is the mass matrix.
\\
...
First, the segment transport is defined by an array of zeros that is generated for the total 16 segments.
The ramp is then defined over the transport over 15 to 16. The ion starts in an initial position of 0.003 meters, which is done by setIons.
The time and position of the ion are defined by t and p respectively, and the ion is then propagated over the segment to be transported.
The voltage of the propegation is then returned and the method gets the potential of the ion.
Since the ion to be shuttled is a Carbon-40 isotope, its mass is used for the energy calculation in the final step.
\\
The energy and plot corresponding to the energy generated during transport are then given below:
...
\begin{subfigure}{0.5\linewidth}
  \centering
  \includegraphics[width=\linewidth]{Figures/unprecompexc.png}
  \subcaption{The excitation induced in the unprecomensated ion shuttling ramp. Some excitation begins during the first decent of the graph, and remains in oscillation throughout the rest of the ion propegation.}
\end{subfigure}
\quad
\begin{subfigure}{0.5\linewidth}
  \centering
  \includegraphics[width=\linewidth]{Figures/unp.png}
  \subcaption{Original unprecompensated waveform ramp from segment 15 to 16 to 15.}
\end{subfigure}
\caption{Excitation induced by the unprecompensated ramp.}
\end{figure}

The final ion's energy is:

\begin{minted}{python}
    energy/hbar_omega: [75.96217604]
    Start position: -0.0003
    End position: -0.00029990713363558776
\end{minted}

This ramp transport the ion from segment 15 to 16. In the graph, this corresponds to an initial peak (15 to 16), a wait time (variable) and then back down (16 to 15). Thus, the ion should end in the same position. Here, the ion's initial and final positions are near-perfectly the same.

\subsection{Precompensated Ramp Simulation}

Next, the precompensated ramp is simulated. This uses the same process as the original case, just with a different input ramp.

\begin{figure}[H]
\centering
\begin{subfigure}{0.5\linewidth}
  \centering
  \includegraphics[width=\linewidth]{Figures/precompexc.png}
  \subcaption{The excitation induced in the precomensated ion shuttling ramp. Large amounts of excitation begin during the first ascent of the graph, and remains in oscillation throughout the rest of the ion propegation.}
\end{subfigure}
\quad
\begin{subfigure}{0.5\linewidth}
  \centering
  \includegraphics[width=\linewidth]{Figures/pc.png}
  \subcaption{Precompensated waveform ramp from segment 15 to 16 to 15.}
\end{subfigure}
\caption{Excitation induced by the unprecompensated ramp.}
\end{figure}

As seen here, the ramp has a much bigger excitation than the original uncompensated ramp. This may be due to the fact that the precompensated ramp has larger rates of change and deals with higher frequencies, which results in higher energies.

\subsection{Ideal Precompensated Ramp and Calculating Ramp Frequency}

To achieve the goal of a theoretically ideal voltage ramp that is precompensated with zero excitation, the frequency of the ion in the minimum of the potential must be held constant throughout transport. This means, the same initial and final frequencies. 
To calculate the ion's (angular) frequency, start with the given potential energy profile $U(x)$. This is returned from the function `redtrap.getPot(x, 0)`, and is in eV.
Then, the ion's secular angular frequency $\omega$ at the minimum located at $x_0=0\ \text{m}$ can be computed.

\subsubsection{Harmonic Approximation Near the Minimum}
...
\begin{center}
    $ \;\omega \;=\; \sqrt{\dfrac{U''(x_0)}{m}}\; $
\end{center}

If you had electric potential $\Phi(x)$ in volts (not energy), then $U(x)=q\,\Phi(x)$ and
\begin{center}
    $ > \;\omega^2 \;=\; \dfrac{q}{m}\,\Phi''(x_0)\; $
\end{center}

\subsubsection{Estimating the Curvature}

For a quadratic fit (robust), fit a parabola in a small symmetric window around $x_0=0$:

\begin{center}
    $ U(x)\;\approx\; a\,x^2 + b\,x + c\quad \Longrightarrow \quad U''(0) = 2a $
\end{center}

On an evenly spaced grid with step \(dx\),
\begin{center}
    $ U''(x_i)\;\approx\;\dfrac{U_{i+1}-2U_i+U_{i-1}}{dx^2} $
\end{center}

\subsubsection{Units}
The getPot method returns energy in eV, and $U''$ is kept in eV $\cdot$ m $^2$.
The mass is expressed in eV $\cdot$ s $^2$ / m $^2$ so that $\sqrt{U''/m}$ yields rad/s directly:
\begin{center}
    massunit = $\dfrac{931.49410242\times 10^6}{c^2}, \; \; \; \; m_{^{40} \mathrm{Ca}^+} \;=\; 39.962590866 \times $ massunit.
\end{center}
...
\begin{itemize}
    ...
    \begin{center}
        $ \omega \;=\; \sqrt{\dfrac{U''(0)}{m}}$ in rad/sec
    \end{center}

    \item Frequency:
    \begin{center}
        $f \;=\; \dfrac{\omega}{2\pi} $ Hz
    \end{center}
\end{itemize}

For the example case, this initial frequency of the trapped ion is:

\begin{minted}{python}
#The final frequency is 424 KHz
\end{minted}

For zero excitation, the frequency of the ion throughout the applied waveform must remain zero.
For this, a ramp with an ideal shape that results in no excitation must be found.

\subsection{Theoretically Ideal Ramp}
For a theoretically zero excitation ramp, as mentioned before, the precompensated waveform must have a smoother rate of change. An attempt at this is shown below, where waveforms '1.1' and '2.1' are an attempt at smoother precompensation, while '1.2' and '2.2' are the original precompensated waveforms.
It is important to note that the wait time feature is removed for this example, which is why the waveform ascends and descents without being constant in the middle as done previously.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{Figures/waveformcomparison.png}
\caption{Waveforms '1.1' and '1.2' are modified to give a precompensated result with lower derivative minimums and maximums. Waveforms '2.1' and '2.2' are the originally precompensated cases.}
\end{figure}

For these new waveforms, the resulting precompensated ramps are as follows:

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{Figures/modifiedresults.png}
\caption{These are the outputs for the case where modified waveforms ('1.1' and '1.2') are used instead.}
\end{figure}

The resulting precompensated waveform corresponding to the modified input is a lot smoother and peaks less than the original precompensated waveform. This smoother rate of change suggests a lower excitation. Below is given a comparison
of the original, precompensated, and excitation waveforms:

\begin{figure}[H]
\centering
\begin{tabular}{cc}
    \includegraphics[width=0.45\linewidth]{Output/2.1.png} &
    \includegraphics[width=0.45\linewidth]{Output/1.1.png} \\
    \includegraphics[width=0.45\linewidth]{Output/1.2.png} &
    \includegraphics[width=0.45\linewidth]{Output/2.2.png} \\
    \includegraphics[width=0.45\linewidth]{Output/1.3.png} &
    \includegraphics[width=0.45\linewidth]{Output/2.3.png}
\end{tabular}
\caption{The first column is the original input, precompensated waveform, and output excitation. The second column is the smoothness-modified input, reprecompensated waveform, and new ouput excitation.}
\end{figure}

As seen in the image above, a smoother precompensated waveform results in less excitation. In the modified input, the corresponding excitation graph no longer peaks at 60 volts but instead at 40 volts. 
With different input waveforms that result in smoother outputs, an ideal input waveform can be reached at which the corresponding outfit has no excitation. This can be verified by keeping the frequency
of the trapped ion constant throughout transport. 
